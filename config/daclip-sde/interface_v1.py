...
# v1版本，手动选择退化复原功能完全由文本编码器生成的degradation_features作为text_context，效果较差
import os
import argparse
import numpy as np
import torch
...
def restore(image, degradation_type=None):
    ...
    if degradation_type is not None and degradation_type != "自动选择":
        ...
        with torch.no_grad(), torch.cuda.amp.autocast():
            ...
            # 如果输入的degradation_type等于图像控制器生成的图像嵌入计算概率最大的退化类型，因该使用自动检测复原？

        # 复原阶段
        LQ_tensor = torch.tensor(image, dtype=torch.float32).permute(2, 0, 1).unsqueeze(0)
        #  permute改变次序
        noisy_tensor = sde.noise_state(LQ_tensor)
        model.feed_data(noisy_tensor, LQ_tensor, text_context=degradation_features, image_context=image_context)
        # 指定输入的degradation_features
        model.test(sde)
        ...
        # 调用函数并打印结果
        degradation_type = find_corresponding_degradation(text_tokens, degradations, max_category)
        print(f"Most likely degradation type: {degradation_type} with a probability of: {max_prob:.4f}")
        ...
        image = restore_image[:, :, [2, 1, 0]]
    # 返回恢复后的图像和输入的退化类型。
        degradation_type = "自动检测识别结果："+degradation_type
